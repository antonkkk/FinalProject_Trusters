...
def test_api_01_update_cd(read_config, read_user_creds, read_contact_temp):
    ...
    response_data = response.json()  # Преобразуем тело ответа в JSON
...
def test_api_02_partial_update_cd(read_config, read_user_creds, read_contact_temp):
    ...
    assert response_data["birthdate"] == read_contact_temp[0]["birthdate"]
    assert response_data["email"] == read_contact_temp[0]["email"]
    assert response_data["phone"] == read_contact_temp[0]["phone"]
    assert response_data["street1"] == read_contact_temp[0]["street1"]
    assert response_data["street2"] == read_contact_temp[0]["street2"]
    assert response_data["city"] == read_contact_temp[0]["city"]
    assert response_data["stateProvince"] == read_contact_temp[0]["stateProvince"]
    assert response_data["postalCode"] == read_contact_temp[0]["postalCode"]
    assert response_data["country"] == read_contact_temp[0]["country"]
